...
public class DownloadPluginThread implements Runnable {
    ...
    public DownloadPluginThread(final String downloadlocation, final String dldetails, final File destination, final TuxTwoLib plugin) {
    ...
    @Override
    public void run() {
        final File tempfile = new File(this.downloadlocation + File.separator + "TuxTwoLib.jar.part");
        try {
            URL website;
